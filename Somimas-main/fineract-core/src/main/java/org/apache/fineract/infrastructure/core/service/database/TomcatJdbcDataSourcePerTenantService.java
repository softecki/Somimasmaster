...
public class TomcatJdbcDataSourcePerTenantService implements RoutingDataSourceService, ApplicationListener<ContextRefreshedEvent> {
    ...
    /**
     * Initializes (or reuses) a datasource pool for a newly provisioned tenant.
     */
    public void initializeTenant(FineractPlatformTenant tenant) {
        initializeDataSourceConnection(tenant);
    }

    /**
     * Evicts and closes a tenant datasource pool after credential rotation or decommission.
     */
    public void evictTenantDataSource(Long connectionId) {
        if (connectionId == null) {
            return;
        }
        DataSource removed = TENANT_TO_DATA_SOURCE_MAP.remove(connectionId);
        if (removed instanceof AutoCloseable closeable) {
            try {
                closeable.close();
            } catch (Exception e) {
                log.warn("Failed to close datasource for connectionId {}", connectionId, e);
            }
        }
    }
}
