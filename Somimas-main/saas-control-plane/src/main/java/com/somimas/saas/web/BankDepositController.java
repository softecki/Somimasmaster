...
import com.somimas.saas.payment.BankDepositService;
...
@RestController
@RequestMapping("/organizations/{id}/bank-deposits")
public class BankDepositController {
    ...
    @PostMapping
    public BankDepositResponse submitDeposit(@PathVariable("id") Long organizationId,
            @Valid @RequestBody BankDepositSubmitRequest request) {
        organizationAuthorizationService.requireMembership(SecurityUtils.currentIdentityId(), organizationId);
        return bankDepositService.submit(organizationId, SecurityUtils.currentIdentityId(), request);
    }
}
